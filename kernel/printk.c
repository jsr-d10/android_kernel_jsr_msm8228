...
#define LOG_CPUID_UNK   99
#define LOG_CPUID_TAG   ((const char *)1)
...
static void cont_flush(enum log_flags flags)
{
	...
	if (cont.cons) {
		...
		log_store(cont.facility, cont.level, flags | LOG_NOCONS,
			  cont.ts_nsec, LOG_CPUID_TAG, (u16)cont.cpuid, cont.buf, cont.len);
		cont.flags = flags;
		cont.flushed = true;
	} else {
		/*
		...
		log_store(cont.facility, cont.level, flags, 0,
			  LOG_CPUID_TAG, (u16)cont.cpuid, cont.buf, cont.len);
		cont.len = 0;
	}
}
